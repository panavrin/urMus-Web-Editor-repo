...
local regionmoviecount = 1
local globalmoviecount = 1
...
function PlaybackRecord(self)
    ...
    WriteMovie("moviefilename"..globalmoviecount..".mp4", background:Left(),background:Bottom(),background:Right(),background:Top())
end
...
function StopRecord(self)
    ...
    newregion.t:SetTexture(255,255,255,255)
--    newregion.t:SetTexture(DocumentPath("moviefilename"..globalmoviecount..".mp4"))
    globalmoviecount = globalmoviecount + 1
end
...
function RegionRecord(self)
    ...
    if record == 0 then
        if selectindex ~= nil then
            ...
            regions[selectindex].t:WriteMovie("regionfilename"..regionmoviecount..".mp4")
            record = 1
        else
            DPrint("no region selected")
        end
        ...
        newregion = CreateRegionAt(ScreenWidth()/3,ScreenHeight()*2/3)
        newregion.t:SetTexture(255,255,255,255)
        if regionmoviecount > 2 then
            newregion.t:SetTexture(DocumentPath("regionfilename"..(regionmoviecount-1)..".mp4"))
        end
        regionmoviecount = regionmoviecount + 1
        record = 0
    end
end
...
for i=1, 4 do
    ...
    menu_buttons[i].t:SetTexture(pics[i])  
