...
bg2 = Region()
bg2.t = bg2:Texture()
bg2.t:SetGradientColor("HORIZONTAL",0,0,0,200,0,128,0,200)
bg2:Show()
...
txt = {"Kick","Clap","Hat","Crash","Loop :","Sine",
  "Square","Saw","Scratch","Write :"}

function tap(self)
  if(self.write == "norm" and self.loop == "norm" and self.sel == "norm") then
      self.t:SetSolidColor(0,0,255,180)
  ...
  if(self.write == "off") then
      self.t:SetSolidColor(255,0,0,180)
      self.write = "on"
      writePush:Push(1.0)
  end
  if(self.loop == "on") then
      self.t:SetSolidColor(64,64,64,180)
      self.loop = "off"
  ...
  if(self.sel == "on") then
      ...
      if(pads[i].sel == "on") then
        pads[i].t:SetSolidColor(64,64,64,180)
        pads[i].sel = "off"
      end
      end
      self.sel = "on"
...
function release(self)
  ...
  if((self.loop == "on") or (self.sel == "on")) then
      self.t:SetSolidColor(0,255,0,180)
  end
end
...
for i = 1,10 do
  ...
  pad.tl:SetFontHeight(16)
  pad.tl:SetColor(255,255,255,255)
  pad.tl:SetHorizontalAlign("JUSTIFY")
  pad.tl:SetShadowColor(0,0,0,255)
  pad.tl:SetShadowOffset(1,1)
  pad.tl:SetShadowBlur(1)
  pad:Show()
  pad:SetWidth(bg:Width()*5/32)
  pad:SetHeight(bg:Width()*5/32)
  pad.t:SetTexture("pad.png")
  pad.t:SetSolidColor(64,64,64,180);
...
disc.t = disc:Texture()
disc:Show()
disc:SetWidth(bg:Width()/1.05)
disc:SetHeight(bg:Width()/1.05)
...
function turnDisc(self,offset)
    ...
    if(1.5*self.v > 9*ScreenWidth()/111 and 1.5*self.v < 102*ScreenWidth()/111) then
        fader:SetAnchor("BOTTOMLEFT",bar,"BOTTOMLEFT",1.5*self.v-287*ScreenWidth()/3552,0)
    end
    if(self.dir == "cw") then
...
function rotXY(self, x, y, dx, dy)
    ...
    self.arg = self.arg + dangle
--    self.arg = self.arg+math.atan2(cy,cx) -- +math.pi
    --self.t:SetRotation(self.arg)
    turnDisc(self,0)
end
...
function spin(self, elapsed)
  bg2.t:SetGradientColor("HORIZONTAL",0,0,0,200,0,self.v/3,0,200)
  disc.t:SetSolidColor(255-self.v/2,255-self.v/2,255,255)
  local offset = 0
...
bar.t = bar:Texture(64,64,64,180)
...
function ConfigPage()
    ...
    if not pagebutton2 then
        pagebutton2=Region()
        pagebutton2:SetWidth(pagersize)
        pagebutton2:SetHeight(pagersize)
        pagebutton2:SetLayer("TOOLTIP")
        pagebutton2:SetAnchor('BOTTOMLEFT',ScreenWidth()-pagersize-4,pagersize+4); 
        pagebutton2:EnableClamping(true)
        pagebutton2:Handle("OnDoubleTap", MainPage)
        pagebutton2.texture = pagebutton2:Texture("circlebutton-16.png")
        pagebutton2.texture:SetBlendMode("BLEND")
        pagebutton2:EnableInput(true)
        pagebutton2:Show()
    end
end
...
pagebutton:SetAnchor('BOTTOMLEFT',ScreenWidth()-pagersize-4,pagersize+4); 
