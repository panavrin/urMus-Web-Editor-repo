...
function RotateMe(self, elapsed)
    angle = angle + speed*elapsed
    self.t:SetRotation(angle)
end

function TouchDown(self)
local region = CreateorRecycleregion('region', 'backdrop', UIParent)
region.t:SetTexture(255,255,255,255)
region.t:SetTexture("urMus-Intro_1.mp4");
--region.t:SetTexture(DocumentPath("urMus-Intro.mp4"));
--region.t:SetTexture("Ornament1.png");
--region.t:UseCamera()
--region.t:WriteMovie("urMus-test.mp4");
region:Show()
region:SetWidth(ScreenWidth())
region:SetHeight(ScreenHeight()/2)
region:EnableMoving(true)
region:EnableResizing(true)
region:EnableInput(true)
region:Handle("OnDoubleTap", RecycleSelf)
--region:Handle("OnUpdate", RotateMe)
local x,y = InputPosition()
