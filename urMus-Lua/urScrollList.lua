...
function urScrollList:CreatescrollRegion(text1, text2, text3, callback, color, path, icontexture, backdroptexture, position)
	...
	if not region.text1region then
		...
		local dummy = region.text1region and region.text1region:SetAnchor("TOPLEFT", region, "TOPLEFT", 0,0)
		local dummy = region.text2region and region.text2region:SetAnchor("TOPRIGHT", region, "TOPRIGHT", 0, 0)
		local dummy = region.text3region and region.text3region:SetAnchor("BOTTOMRIGHT", region, "BOTTOMRIGHT", 0,0)
	else
...
--	if label and label~=region:tl:Label() then
	local region
